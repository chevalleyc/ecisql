...
public class Query {
	...
	protected static Logger log = LogManager.getLogger(Query.class);
	...
	protected final ExemplifyMode tag_mode; //default
...
//	private Gson gson = new Gson();
	...
	protected Query(ExemplifyMode mode) throws IllegalAccessException {
		this.allElements = false;
        this.tag_mode = mode;
//		initTags();
	}

	protected Query(boolean allElements) throws IllegalAccessException {
		this.allElements = allElements;
		this.tag_mode = ExemplifyMode.ANY_FIELD;
//		initTags();
	}

    protected Query(ExemplifyMode mode, boolean allElements) throws IllegalAccessException {
        this.allElements = allElements;
        this.tag_mode = mode;
//		initTags();
    }
    ...
    protected Query() throws IllegalAccessException {
		this.allElements = false;
        this.tag_mode = ExemplifyMode.ANY_FIELD;
//		initTags();
	}
	...
	protected String getNodeTag(String prefix, Locatable node, Object container) {
		...
		case ANY_FIELD:
			if (node == null)
				...
				if (!container.getClass().equals(MultiValueMap.class) && (!(path.startsWith(Query.TAG_DESCRIPTION)))){
					if (path.contains("[openEHR-") || path.contains(Query.TAG_ACTIVITIES) || path.contains(Query.TAG_ITEMS) || path.contains(Query.TAG_EVENTS)) {
					//expand name in key
...
//				break;
	...
	private List<String> attributes(GraphQLObjectType objectType, CompositionSerializer.WalkerOutputMode mode){
		List<String> fields = new ArrayList<>();

		for (GraphQLFieldDefinition definition: objectType.getFieldDefinitions()){
			fields.add(definition.getName());
		}
